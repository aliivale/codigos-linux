...
void inicializar_terreno(tmat terreno, juego_t juego){
    ...
    {
        terreno[juego.bombas[b].posicion.fil][juego.bombas[b].posicion.col] = 'B';
        
    }
    // 2. COLOCAMOS HERRAMIENTAS
    for (int h = 0; h < juego.tope_herramientas; h++)
    {
        terreno[juego.herramientas[h].posicion.fil][juego.herramientas[h].posicion.col] = juego.herramientas[h].tipo;
    }
    // 3. COLOCAMOS FAMILIARES
    for (int fam = 0; fam < juego.tope_familiares; fam++)
    {
        terreno[juego.familiares[fam].posicion.fil][juego.familiares[fam].posicion.col] = juego.familiares[fam].inicial_nombre;
    }
